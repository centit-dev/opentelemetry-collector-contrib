...
// Mutate calls f(ctx, m).
func (f QueryKeyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.QueryKeyMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.QueryKeyMutation", m)
}

// The QueryValueFunc type is an adapter to allow the use of ordinary
// function as QueryValue mutator.
type QueryValueFunc func(context.Context, *ent.QueryValueMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f QueryValueFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.QueryValueMutation); ok {
		return f(ctx, mv)
	}
